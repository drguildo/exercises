...
public final class StdDraw3D implements MouseListener, MouseMotionListener,
    MouseWheelListener, KeyListener, ActionListener, ChangeListener,
  ...
  private static JRadioButtonMenuItem orbitModeButton, fpsModeButton,
      airplaneModeButton, lookModeButton, fixedModeButton;
  ...
  private static BranchGroup rootGroup, lightGroup, soundGroup, fogGroup,
      appearanceGroup;
  ...
  // Default shape flags.
  private static final int PRIMFLAGS = Primitive.GENERATE_NORMALS
      + Primitive.GENERATE_TEXTURE_COORDS;

  // Infinite bounding sphere.
  private static final BoundingSphere INFINITE_BOUNDS = new BoundingSphere(
      new Point3d(0.0, 0.0, 0.0), 1e100);
  ...
  private static JMenuBar createMenuBar() {
    ...
    loadButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L, Toolkit
        .getDefaultToolkit().getMenuShortcutKeyMask()));
    fileMenu.add(loadButton);

    saveButton = new JMenuItem(" Save Image...  ");
    saveButton.addActionListener(std);
    saveButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, Toolkit
        .getDefaultToolkit().getMenuShortcutKeyMask()));
    fileMenu.add(saveButton);

    save3DButton = new JMenuItem(" Export 3D Scene...  ");
    save3DButton.addActionListener(std);
    save3DButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, Toolkit
        .getDefaultToolkit().getMenuShortcutKeyMask()));
    ...
    quitButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, Toolkit
        .getDefaultToolkit().getMenuShortcutKeyMask()));
    ...
    fpsModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_2, Toolkit
        .getDefaultToolkit().getMenuShortcutKeyMask()));
    ...
    lookModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_4, Toolkit
        .getDefaultToolkit().getMenuShortcutKeyMask()));
    ...
    SpinnerNumberModel snm2 = new SpinnerNumberModel(DEFAULT_NUM_DIVISIONS, 4,
        4000, 5);
  ...
  private static Texture createTexture(String imageURL) {
    ...
    try {
      loader = new TextureLoader(imageURL, "RGBA", TextureLoader.Y_UP,
          new Container());
    } catch (Exception e) {
  ...
  public void mouseWheelMoved(MouseWheelEvent e) {
    ...
    if ((cameraMode == ORBIT_MODE)
        && (view.getProjectionPolicy() == View.PARALLEL_PROJECTION)) {
      camera.moveRelative(0, 0, notches * zoom / 20);
    }
  }

  private static void mouseMotionEvents(MouseEvent e, double newX, double newY,
      boolean dragged) {
    ...
    if ((cameraMode == ORBIT_MODE)
        && (dragged && isKeyPressed(KeyEvent.VK_ALT))
        && (view.getProjectionPolicy() == View.PARALLEL_PROJECTION)) {
      camera.moveRelative(0, 0, (newY - mouseY) * zoom / 50);
      return;
    }
  }
  ...
  private static void moveEvents(int time) {
    ...
    if (cameraMode == FPS_MODE) {
      ...
      if (isKeyPressed('W') || isKeyPressed(KeyEvent.VK_UP))
        camera.moveRelative(0, 0, move * 3);
      if (isKeyPressed('S') || isKeyPressed(KeyEvent.VK_DOWN))
        camera.moveRelative(0, 0, -move * 3);
      if (isKeyPressed('A') || isKeyPressed(KeyEvent.VK_LEFT))
        camera.moveRelative(-move, 0, 0);
      if (isKeyPressed('D') || isKeyPressed(KeyEvent.VK_RIGHT))
        camera.moveRelative(move, 0, 0);
      if (isKeyPressed('Q') || isKeyPressed(KeyEvent.VK_PAGE_UP))
        camera.moveRelative(0, move, 0);
  ...
  private static void save3DAction() {
    ...
    FileDialog chooser = new FileDialog(StdDraw3D.frame,
        "Save as a 3D file for loading later.", FileDialog.SAVE);
    chooser.setVisible(true);
    String filename = chooser.getFile();
    if (filename != null) {
      StdDraw3D.saveScene3D(chooser.getDirectory() + File.separator
          + chooser.getFile());
    }
  ...
  private static void saveAction() {
    ...
    if (filename != null) {
      StdDraw3D.save(chooser.getDirectory() + File.separator
          + chooser.getFile());
    }
  ...
  public static void setBackground(String imageURL) {
    ...
    ImageComponent2D imageComp = new ImageComponent2D(
  ...
  public static void setBackgroundSphere(String imageURL) {
    ...
    Sphere sphere = new Sphere(1.1f,
        Primitive.GENERATE_NORMALS | Primitive.GENERATE_NORMALS_INWARD
            | Primitive.GENERATE_TEXTURE_COORDS, numDivisions);
  ...
  private static void playAmbientSound(String filename, double volume,
      boolean loop) {
  ...
  public static Light pointLight(double x, double y, double z, Color col,
    ...
    light.setAttenuation(1.0f, linearFade / scale, quadraticFade
        / (scale * scale));
  ...
  public static Vector3D randomDirection() {
    ...
    return new Vector3D(Math.cos(theta) * Math.sin(phi), Math.sin(theta)
   ...
   */
  public static Shape sphere(double x, double y, double z, double r, double xA,
      double yA, double zA) {
   ...
   */
  public static Shape sphere(double x, double y, double z, double r, double xA,
      double yA, double zA, String imageURL) {
   ...
   */
  public static Shape ellipsoid(double x, double y, double z, double w,
      double h, double d) {
   ...
   */
  public static Shape ellipsoid(double x, double y, double z, double w,
      double h, double d, double xA, double yA, double zA) {
   ...
   */
  public static Shape ellipsoid(double x, double y, double z, double w,
      double h, double d, String imageURL) {
   ...
   */
  public static Shape ellipsoid(double x, double y, double z, double w,
      double h, double d, double xA, double yA, double zA, String imageURL) {

    Sphere sphere = new Sphere(1, PRIMFLAGS, numDivisions);
    sphere.setAppearance(createAppearance(imageURL, true));
    return primitive(sphere, x, y, z, new Vector3d(xA, yA, zA), new Vector3d(w,
        h, d));
  }
   ...
   */
  public static Shape wireEllipsoid(double x, double y, double z, double w,
      double h, double d) {
   ...
   */
  public static Shape cube(double x, double y, double z, double r, double xA,
      double yA, double zA) {
  ...
  public static Shape wireCube(double x, double y, double z, double r) {
    ...
    double[] xC = new double[] { x + r, x + r, x - r, x - r, x + r, x + r,
        x + r, x - r, x - r, x + r, x + r, x + r, x - r, x - r, x - r, x - r };
    double[] yC = new double[] { y + r, y - r, y - r, y + r, y + r, y + r,
        y - r, y - r, y + r, y + r, y - r, y - r, y - r, y - r, y + r, y + r };
    double[] zC = new double[] { z + r, z + r, z + r, z + r, z + r, z - r,
   ...
   */
  public static Shape box(double x, double y, double z, double w, double h,
      double d) {
   ...
   */
  public static Shape box(double x, double y, double z, double w, double h,
      double d, double xA, double yA, double zA) {
   ...
   */
  public static Shape box(double x, double y, double z, double w, double h,
      double d, double xA, double yA, double zA, String imageURL) {
    ...
    com.sun.j3d.utils.geometry.Box box = new com.sun.j3d.utils.geometry.Box(
        dimensions.x, dimensions.y, dimensions.z, PRIMFLAGS, ap, numDivisions);
   ...
   */
  public static Shape wireBox(double x, double y, double z, double w, double h,
      double d) {
   ...
   */
  public static Shape cylinder(double x, double y, double z, double r,
      double h, double xA, double yA, double zA) {
   ...
   */
  public static Shape cylinder(double x, double y, double z, double r,
      double h, double xA, double yA, double zA, String imageURL) {

    Appearance ap = createAppearance(imageURL, true);
    Vector3f dimensions = createVector3f(r, h, 0);
    Cylinder cyl = new Cylinder(dimensions.x, dimensions.y, PRIMFLAGS,
        numDivisions, numDivisions, ap);
   ...
   */
  public static Shape wireCylinder(double x, double y, double z, double r,
      double h) {
   ...
   */
  public static Shape cone(double x, double y, double z, double r, double h,
      double xA, double yA, double zA, String imageURL) {

    Appearance ap = createAppearance(imageURL, true);
    Vector3f dimensions = createVector3f(r, h, 0);
    Cone cone = new Cone(dimensions.x, dimensions.y, PRIMFLAGS, numDivisions,
        numDivisions, ap);
   ...
   */
  public static Shape wireCone(double x, double y, double z, double r,
      double h, double xA, double yA, double zA) {
   ...
   */
  private static Shape primitive(Primitive shape, double x, double y, double z,
      Vector3d angles, Vector3d scales) {
  ...
  public static Shape points(double[] x, double[] y, double[] z) {
    ...
    GeometryArray geom = new PointArray(coords.length,
   ...
   */
  public static Shape line(double x1, double y1, double z1, double x2,
      double y2, double z2) {

    return lines(new double[] { x1, x2 }, new double[] { y1, y2 },
        new double[] { z1, z2 });
  }
  ...
  public static Shape lines(double[] x, double[] y, double[] z) {
    ...
    GeometryArray geom = new LineStripArray(coords.length,
        GeometryArray.COORDINATES, new int[] { coords.length });
    geom.setCoordinates(0, coords);
   ...
   */
  public static Shape tube(double x1, double y1, double z1, double x2,
      double y2, double z2, double r) {
   ...
   */
  private static Shape polygon(double[] x, double[] y, double[] z,
      boolean filled) {

    Point3f[] coords = constructPoint3f(x, y, z);
    GeometryArray geom = new TriangleFanArray(coords.length,
        GeometryArray.COORDINATES, new int[] { coords.length });
    geom.setCoordinates(0, coords);
  ...
  private static Shape triangles(double[][] points, boolean filled) {
    ...
    for (int i = 0; i < size; i++) {
      coords[3 * i] = new Point3f(createVector3f(points[i][0], points[i][1],
          points[i][2]));
      coords[3 * i + 1] = new Point3f(createVector3f(points[i][3],
          points[i][4], points[i][5]));
      coords[3 * i + 2] = new Point3f(createVector3f(points[i][6],
          points[i][7], points[i][8]));
    }
  ...
  private static Shape triangles(double[][] points, Color[] colors,
      boolean filled) {
    int size = points.length;
    Point3f[] coords = new Point3f[size * 3];
  ...
  public static Shape text3D(double x, double y, double z, String text,
    ...
    javax.media.j3d.Text3D t = new javax.media.j3d.Text3D(font3D, text,
        createPoint3f(x, y, z));
   ...
   */
  private static Shape shape(Shape3D shape, boolean fill,
      Transform3D transform, boolean custom) {

    Appearance ap;

    if (custom)
      ap = createCustomAppearance(fill);
    else
      ap = createAppearance(null, fill);
  ...
  public static void overlayPixel(double x, double y) {
    getGraphics2D(offscreenImage).fillRect(Math.round(scaleX(x)),
  ...
  public static void overlayPoint(double x, double y) {
    ...
    if (r <= 1)
      ...
      getGraphics2D(offscreenImage).fill(
          new Ellipse2D.Double(scaleX(x) - r / 2, scaleY(y) - r / 2, r, r));
  }
  ...
  public static void overlayLine(double x0, double y0, double x1, double y1) {
    getGraphics2D(offscreenImage).draw(
   ...
   */
  public static void overlayEllipse(double x, double y, double semiMajorAxis,
   ...
   */
  public static void overlayFilledEllipse(double x, double y,
   ...
   */
  public static void overlayArc(double x, double y, double r, double angle1,
      double angle2) {
    if (r < 0)
    ...
    if (ws <= 1 && hs <= 1)
      ...
      getGraphics2D(offscreenImage).draw(
          new Arc2D.Double(xs - ws / 2, ys - hs / 2, ws, hs, angle1, angle2
              - angle1, Arc2D.OPEN));
  }
   ...
   */
  public static void overlayRectangle(double x, double y, double halfWidth,
      double halfHeight) {
    if (halfWidth < 0)
   ...
   */
  public static void overlayFilledRectangle(double x, double y,
  ...
  public static void overlayPicture(double x, double y, String s) {
    ...
    getGraphics2D(offscreenImage).drawImage(image,
        (int) Math.round(xs - ws / 2.0), (int) Math.round(ys - hs / 2.0), null);
  }
   ...
   */
  public static void overlayPicture(double x, double y, String s, double w,
      double h) {
    Image image = getImage(s);
    double xs = scaleX(x);
    double ys = scaleY(y);
    ...
    if (ws <= 1 && hs <= 1)
      ...
      getGraphics2D(offscreenImage).drawImage(image,
          (int) Math.round(xs - ws / 2.0), (int) Math.round(ys - hs / 2.0),
  ...
  private static Graphics2D getGraphics2D(BufferedImage image) {
    ...
    BasicStroke stroke = new BasicStroke(penRadius, BasicStroke.CAP_ROUND,
  ...
  private static void infoDisplay() {
    ...
    String s = "(" + df.format(pos.x) + "," + df.format(pos.y) + ","
        + df.format(pos.z) + ")";
    g.setColor(BLACK);
    ...
    String s2 = "(" + df.format(rot.x) + "," + df.format(rot.y) + ","
        + df.format(rot.z) + ")";
    g.setColor(BLACK);
    ...
    else if (cameraMode == FPS_MODE)
      mode = "Camera: FPS_MODE";
    else if (cameraMode == AIRPLANE_MODE)
      mode = "Camera: AIRPLANE_MODE";
    else if (cameraMode == LOOK_MODE)
      mode = "Camera: LOOK_MODE";
    else if (cameraMode == FIXED_MODE)
    ...
    double d = b / 4;
    g.draw(new Line2D.Double(scaleX(d + center), scaleY(0 + center), scaleX(-d
        + center), scaleY(0 + center)));
    g.draw(new Line2D.Double(scaleX(0 + center), scaleY(d + center),
        scaleX(0 + center), scaleY(-d + center)));
  ...
  public static void save(String filename) {
    ...
    ImageComponent2D imageComp = new ImageComponent2D(
        ImageComponent.FORMAT_RGB, buf);
    javax.media.j3d.Raster ras = new javax.media.j3d.Raster(new Point3f(-1.0f,
        -1.0f, -1.0f), javax.media.j3d.Raster.RASTER_COLOR, 0, 0, width,
        height, imageComp, null);
    ...
    // reference: http://archives.java.sun.com/cgi-bin/wa?A2=ind0404&L=java2d-interest&D=0&P=2727
      ...
      newRaster = raster.createWritableChild(0, 0, width, height, 0, 0,
          new int[] { 0, 1, 2 });
      BufferedImage rgbBuffer = new BufferedImage(image.getColorModel(),
  ...
  public static void saveScene3D(String filename) {
    ...
    try {
      SceneGraphFileWriter writer = new SceneGraphFileWriter(file, universe,
          false, "3D scene saved from StdDraw3D.", null);
  ...
  public static void loadScene3D(String filename) {
    ...
    try {
      ...
      System.out
          .println("Branch graph count = " + reader.getBranchGraphCount());
  ...
  */
  private static class Transformable {
    ...
    public void rotateAxis(Vector3D axis, double angle) {
      ...
      AxisAngle4d aa = new AxisAngle4d(aRel.x, aRel.y, aRel.z,
  ...
  public static void setCameraOrientation(double xAngle, double yAngle,
      double zAngle) {
  ...
  public static void setCamera(double x, double y, double z, double xAngle,
  ...
  */
  ...
  public static class Vector3D {
    ...
    */
    ...
    public double angle(Vector3D that) {
      return Math.toDegrees(Math.acos(this.dot(that)
          / (this.mag() * that.mag())));
    }
  ...
  public static void main(String[] args) {
    ...
    StdDraw3D.overlayText(0, -0.95,
        "You should see rotating text. Drag the mouse to orbit.");

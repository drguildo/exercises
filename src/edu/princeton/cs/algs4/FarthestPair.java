...
public class FarthestPair {
  ...
  public FarthestPair(Point2D[] points) {
    ...
    int M = 0;
    for (Iterator<Point2D> iterator = graham.hull().iterator(); iterator
        .hasNext();) {
      M++;
    }
    ...
    int k = 2;
    while (Point2D.area2(hull[M], hull[k + 1], hull[1]) > Point2D.area2(
    ...
    for (int i = 1; i <= k; i++) {
      ...
      while ((j < M)
          && Point2D.area2(hull[i], hull[j + 1], hull[i + 1]) > Point2D.area2(
              hull[i], hull[j], hull[i + 1])) {
        j++;
        // StdOut.println(hull[i] + " and " + hull[j] + " are antipodal");
  ...
  public static void main(String[] args) {
    ...
    StdOut.println(farthest.distance() + " from " + farthest.either() + " to "
        + farthest.other());
  }
